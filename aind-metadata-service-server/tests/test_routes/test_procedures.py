...
class TestRoute:
    ...
    @patch("aind_tars_service_async_client.DefaultApi.get_viral_prep_lots")
    @patch("aind_tars_service_async_client.DefaultApi.get_viruses")
    def test_get_procedures_concurrent_protocol_fetches(
        self,
        mock_get_viruses: AsyncMock,
        mock_get_viral_prep_lots: AsyncMock,
        mock_get_protocols: AsyncMock,
        ...
        """Tests that protocol fetches happen concurrently."""
        # Setup mocks with data that requires protocol fetching
        mock_labtracks.return_value = [
            LabTracksTask(
                id="00000",
                type_name="Perfusion Gel",
        ...
        response = client.get("api/v2/procedures/000000")
        assert response.status_code == 200
        assert mock_get_protocols.call_count >= 0
    ...
    def test_get_procedures_concurrent_tars_fetches(
        ...
        # Setup data with virus strains that need TARS lookups
        mock_nsb2019.return_value = []
        mock_nsb2023.return_value = [
            NSB2023List(
                Date_x0020_of_x0020_Surgery="2022-01-03T00:00:00Z",
        ...
        # Verify concurrent TARS calls were made
        assert mock_get_viral_prep_lots.call_count == 1
        assert mock_get_viruses.call_count == 1
    ...
    @patch("aind_tars_service_async_client.DefaultApi.get_viral_prep_lots")
    @patch("aind_tars_service_async_client.DefaultApi.get_viruses")
    def test_get_procedures_multiple_virus_strains_concurrent(
        self,
        mock_get_viruses: AsyncMock,
        mock_get_viral_prep_lots: AsyncMock,
        mock_get_protocols: AsyncMock,
        ...
        """Tests that multiple virus strains are fetched concurrently."""
        mock_labtracks.return_value = []
        mock_las.return_value = []
        mock_get_perfusions.return_value = []
        mock_get_protocols.return_value = []
                ...
                return [
                    PrepLotData(
                        ...
                        viral_prep=ViralPrep(
                            virus=VirusData(
                                aliases=[
                                    Alias(is_preferred=True, name="v_456")
                                ]
                            )
                        ),
                    )
                ...
                return [
                    PrepLotData(
                        ...
                        viral_prep=ViralPrep(
                            virus=VirusData(
                                aliases=[
                                    Alias(is_preferred=True, name="v_123")
                                ]
                            )
                        ),
                    )
        ...
        # Verify multiple virus strains were fetched concurrently
        assert mock_get_viral_prep_lots.call_count == 2
        # Virus lookups happen for each virus_id found in prep lots
        assert mock_get_viruses.call_count == 2
